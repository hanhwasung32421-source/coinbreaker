...
(() => {
  // D: B(원본 구조)에서 다듬은 레이아웃을 A(캔버스)로 이식한 버전
  // 카드 기본 크기(원본 HTML 기준)
  const CANVAS_W = 400;
  const CANVAS_H = 423;
  ...
  // 좌표 (D: 400x423 캔버스 기준). (B에서 다듬은 원본 카드 기준)
  const POS = {
    padX: 24,
    // drawCardTo에서 y0 = topPercentY + 5를 사용하므로, 최종 상단(약 8px)을 맞추기 위해 3으로 시작
    topPercentY: 3,
    topProfitY: 52,
    sectionStartY: 108,
    rowGap: 72,
  ...
  function getPos() {
    return {
      ...
      sideDx: Math.round(numFrom(els.sideDx, 4)),
  ...
  function bind() {
    ...
    if (els.reset)
      ...
      if (els.padX) els.padX.value = "24";
      if (els.topPercentY) els.topPercentY.value = "3";
      if (els.topProfitY) els.topProfitY.value = "52";
      if (els.percentSignDy) els.percentSignDy.value = "0";
      if (els.sectionStartY) els.sectionStartY.value = "108";
      if (els.rowGap) els.rowGap.value = "72";
      if (els.labelToValueGap) els.labelToValueGap.value = "25";
      if (els.sideDx) els.sideDx.value = "4";
